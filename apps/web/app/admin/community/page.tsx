...
export default function CommunityModerationPage() {
  ...
  return <><Header compact backHref="/admin/reports" backLabel="Kembali ke control room"/><main className="page shell community-admin-page">
    <div className="community-admin-bar"><Link href="/admin/reports">← CONTROL ROOM</Link><Link href="/community" target="_blank">BUKA COMMUNITY ↗</Link><button type="button" disabled={refreshing} onClick={() => void load()}>{refreshing ? "MEMUAT..." : "REFRESH"}</button></div>
    <section className="community-admin-hero"><div><p className="eyebrow">// COMMUNITY MODERATION V1</p><h1>REPORT<br/>QUEUE.</h1><p>Review laporan post dan komentar tanpa membuka identitas pelapor ke publik. Satu keputusan menyelesaikan seluruh laporan aktif pada konten yang sama.</p></div><div><strong>{total}</strong><span>KONTEN PERLU DICEK</span></div></section>
    {error ? <div className="admin-access-error"><span>AKSES TERBATAS</span><h2>QUEUE TIDAK DAPAT DIBUKA.</h2><p>{error}</p><Link href="/login" className="tactical-button">MASUK SEBAGAI ADMIN</Link></div> : loading ? <div className="admin-loading"><span/><p>MEMUAT LAPORAN COMMUNITY...</p></div> : <>
      <div className="community-admin-tabs"><button type="button" aria-selected={tab === "posts"} onClick={() => setTab("posts")}>REPORT POST <strong>{posts.length}</strong></button><button type="button" aria-selected={tab === "comments"} onClick={() => setTab("comments")}>REPORT COMMENT <strong>{comments.length}</strong></button></div>
      {activeItems.length === 0 ? <div className="community-admin-empty"><strong>QUEUE {tab === "posts" ? "POST" : "KOMENTAR"} BERSIH.</strong><p>Belum ada laporan yang menunggu keputusan.</p></div> : tab === "posts" ? <div className="community-admin-list">{posts.map((item) => <article key={item.postId} className="community-admin-card"><header><div><span>POST #{item.postId}</span><strong>{item.reportCount} LAPORAN</strong></div><time>Laporan pertama {formatDate(item.firstReportedAt)}</time></header><AuthorRow author={item.author}/><blockquote>{item.body}</blockquote><Reports reports={item.reports}/><CommunityModerationActions targetId={item.postId} kind="post" onDone={() => void load()}/></article>)}</div> : <div className="community-admin-list">{comments.map((item) => <article key={item.commentId} className="community-admin-card comment"><header><div><span>KOMENTAR #{item.commentId}</span><strong>{item.reportCount} LAPORAN</strong></div><time>Laporan pertama {formatDate(item.firstReportedAt)}</time></header><AuthorRow author={item.author}/><blockquote>{item.body}</blockquote><div className="community-admin-parent"><span>DI POST #{item.postId}</span><p>{item.postBody}</p></div><Reports reports={item.reports}/><CommunityModerationActions targetId={item.commentId} kind="comment" onDone={() => void load()}/></article>)}</div>}
    </>}
  </main><Footer/></>;
}
