...
export function CommunityModerationActions({ targetId, kind, onDone }: { targetId: number; kind: "post" | "comment"; onDone: () => void }) {
  ...
  return <div className="community-moderation-actions">
    ...
    <label><span>ALASAN KEPUTUSAN</span><textarea rows={3} minLength={10} maxLength={500} value={rationale} onChange={(event) => { setRationale(event.target.value); setConfirming(false); }} placeholder={decision === "REMOVE" ? `Jelaskan pelanggaran yang membuat ${kind === "post" ? "post" : "komentar"} harus dihapus.` : "Jelaskan kenapa laporan tidak memerlukan penghapusan."}/><small>{rationale.length}/500 karakter · tersimpan dalam riwayat keputusan admin</small></label>
    {error && <p>{error}</p>}
