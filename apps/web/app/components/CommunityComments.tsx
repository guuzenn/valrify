...
export function CommunityComments({ postId, postAuthorId, initialCount, topReply, user, username, defaultOpen = false, initialComments }: { postId: number; postAuthorId: string; initialCount: number; topReply: CommunityPost["topReply"]; user: SessionUser | null; username: string | null; defaultOpen?: boolean; initialComments?: CommunityComment[] }) {
  ...
  return <section className="community-comments">
    {!open && preview && <article className="community-top-reply"><header><Link href={`/u/${preview.authorUsername}`}><span className="community-comment-avatar">{preview.authorDisplayName.slice(0, 1).toUpperCase()}</span><div><span className="community-comment-name"><strong>{preview.authorDisplayName}</strong>{preview.authorId === postAuthorId && <b>PENULIS</b>}</span><small>@{preview.authorUsername}</small></div></Link><span>TOP REPLY</span></header><p>{preview.body}</p>{preview.likeCount > 0 && <small><HeartIcon filled/> {preview.likeCount} suka</small>}</article>}
