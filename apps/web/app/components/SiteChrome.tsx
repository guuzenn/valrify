...
export function Header({ compact=false }:{compact?:boolean}) {
  return <header className={`site-header shell ${compact?"compact":""}`}><Link className="wordmark" href="/" aria-label="VLRFY beranda">VLRFY<span>{"//"}</span></Link><nav aria-label="Navigasi utama"><Link href="/search">CEK</Link><Link href="/methodology">METODOLOGI</Link><Link href="/admin/reports">REVIEW</Link><Link href="/login">MASUK</Link><Link href="/submit" className="nav-report">LAPORKAN</Link></nav></header>;
}
