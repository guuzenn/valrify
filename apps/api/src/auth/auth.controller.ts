...
@Post("register")register(@Body()body:unknown){return this.auth.register(registerSchema.parse(body));}
@Post("verify-email")verify(@Body()body:{token?:string}){return this.auth.verifyEmail(String(body.token??""));}
@Post("login")@HttpCode(200)async login(@Body()body:unknown,@Res({passthrough:true})response:Response){const result=await this.auth.login(loginSchema.parse(body));response.cookie("vlrfy_session",result.token,cookieOptions());return{user:result.actor};}
