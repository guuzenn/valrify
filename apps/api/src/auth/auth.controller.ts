import { Body,Controller,Get,HttpCode,Post,Req,Res,UseGuards } from "@nestjs/common";import type { Request,Response } from "express";import { loginSchema,registerSchema } from "@valrify/validation";import { parseSchema } from "../validation/parse-schema";import { AuthService } from "./auth.service";import { CurrentActor } from "./current-actor";import { JwtAuthGuard } from "./jwt-auth.guard";import type { AuthActor } from "./auth.types";
...
@Post("login")@HttpCode(200)async login(@Body()body:unknown,@Res({passthrough:true})response:Response){const result=await this.auth.login(parseSchema(loginSchema,body));response.cookie("valrify_session",result.token,cookieOptions());return{user:result.actor};}
@Post("logout")@HttpCode(200)logout(@Res({passthrough:true})response:Response){response.clearCookie("valrify_session",{path:"/"});return{ok:true};}
